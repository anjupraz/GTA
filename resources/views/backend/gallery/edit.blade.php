...
@section('header', '#menu_gallery')
@section('content')
<div class="right_col" role="main">
    <form method="post" action="{{route('gallery.update',['id' => $album->id])}}" class="form-horizontal form-label-left" enctype="multipart/form-data" novalidate>
        @csrf
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-8">
                <div class="x_panel">
                    <div class="x_title">
                        <h2>
                            Gallery<small>[ code : {{$album->code}} ]</small>@if($album->slug)<small>[ slug : <a href="https://gtanepal.org/gallery/{{$album->slug}}" target="_blank"><strong>https://gtanepal.org/gallery/{{$album->slug}}</strong></a> ]</small>@endif
                        </h2>
                    ...
                    <div class="x_content">
                        <div class="item form-group @error('title') bad @enderror">
                            ...
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                <input type="hidden" hidden="hidden" name="code" value="{{old('code',$album->code)}}">
                                <input type="hidden" hidden="hidden" name="slug" value="{{old('slug',$album->slug)}}">
                                <input type="text" id="title" name="title" value="{{old('title',$album->title)}}" required="required" class="form-control col-md-7 col-xs-12">
                            </div>
                            @error('title')
                        ...
                        <div class="item form-group">
                            ...
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                <textarea id="description" name="description" required="required" class="form-control col-md-7 col-xs-12 summernote">{{old('description',$album->description)}}</textarea>
                            </div>
                            @error('description')
                                <div class="alert">{{$message}}</div>
                            @enderror
            ...
            <div class="col-xs-12 col-sm-12 col-md-4">
                <div class="x_panel">
                    <div class="x_title">
                        <h2>
                            Featured Image <small>( 700px * 466px )</small>
                        </h2>
                        <div class="clearfix"></div>
                    </div>
                    <div class="x_content">
                        <div class="item form-group">
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                @if($album->featured_gallery)
                                    <img id="preview" height="100" width="100" src="{{$album->featured_gallery['media']}}" alt="profile" />
                                @else
                        ...
                        <div class="item form-group @error('featured_gallery') bad @enderror">
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                @if($album->featured_gallery)
                                    <input type="hidden" hidden="hidden" name="featured_gallery[id]" value="{{$album->featured_gallery['id']}}">
                                @endif
