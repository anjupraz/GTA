...
@section('header', '#menu_services')
@section('content')
<div class="right_col" role="main">
    <form method="post" action="{{route('services.update',['id' => $service->id])}}" class="form-horizontal form-label-left" enctype="multipart/form-data" novalidate>
        @csrf
        <div class="row">
            <div class="col-xs-12 col-sm-12 col-md-8">
                <div class="x_panel">
                    <div class="x_title">
                        <h2>
                            Services<small>[ code : {{$service->code}} ]</small>@if($service->slug)<small>[ slug : <a href="https://gtanepal.org/service/{{$service->slug}}" target="_blank"><strong>https://gtanepal.org/blog/{{$service->slug}}</strong></a> ]</small>@endif
                        </h2>
                    ...
                    <div class="x_content">
                        <div class="item form-group @error('title') bad @enderror">
                            ...
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                <input type="hidden" hidden="hidden" name="code" value="{{old('code',$service->code)}}">
                                <input type="hidden" hidden="hidden" name="slug" value="{{old('slug',$service->slug)}}">
                                <input type="text" id="title" name="title" value="{{old('title',$service->title)}}" required="required" class="form-control col-md-7 col-xs-12">
                            </div>
                            @error('title')
                        ...
                        <div class="item form-group">
                            ...
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                <textarea id="description" name="description" required="required" class="form-control col-md-7 col-xs-12 summernote">{{old('description',$service->description)}}</textarea>
                            </div>
                            @error('description')
                                <div class="alert">{{$message}}</div>
                            @enderror
                        ...
                        <div class="item form-group @error('category_id') bad @enderror">
                            <select id="category_id" class="form-control col-md-12 col-sm-12 col-xs-12" name="category_id" required="required">
                                @foreach($categoryList as $category)
                                    <option value="{{$category->id}}" @if(old('category_id',$service->category_id)==$category->id) selected @endif>{{$category->title}}</option>
                                @endforeach
                            </select>
                            @error('category_id')
                                <div class="alert">{{$message}}</div>
                ...
                <div class="x_panel">
                    ...
                    <div class="x_content">
                        <div class="item form-group">
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                @if($service->featured_gallery)
                                    <img id="preview" height="100" width="100" src="{{$service->featured_gallery['media']}}" alt="profile" />
                                @else
                        ...
                        <div class="item form-group @error('featured_gallery') bad @enderror">
                            <div class="col-md-12 col-sm-12 col-xs-12">
                                @if($service->featured_gallery)
                                    <input type="hidden" hidden="hidden" name="featured_gallery[id]" value="{{$service->featured_gallery['id']}}">
                                @endif
